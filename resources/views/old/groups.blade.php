        ...
        <div class="swiper-wrapper">
            ...
            <div class="swiper-slide">
                <div class="imgBx">
                    ...
                    <img src="{{$group->image}}" style='width:400px; height:400px;' class="img-responsive">
                </div>
                <div class="details">
                    @if($group->users->contains(Session::get('CurrentUser')->id))
                    <button type="button" class="btn btn-danger"
                        onclick="UnFollowing(this,{{$group->id}},'{{{ csrf_token() }}}')">Leave</button>
                    @elseif($group->waits->contains(Session::get('CurrentUser')->id))
                    <button type="button" class="btn btn-warning" disabled>Wait</button>
                    @else
                    <button type="button" class="btn btn-primary"
                        onclick="Following(this,{{$group->id}},'{{{ csrf_token() }}}')">Join</button>
                    @endif
